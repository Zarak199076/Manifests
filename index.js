...
async function handleUploadAssets(interaction) {
  ...
  try {
    ...
    if (isGame) {
      // Discord's CDN replaces spaces with underscores in uploaded attachment
      // filenames before the bot ever sees them — this undoes that specifically
      // for this path, since it's the one place we use the raw attachment name
      // as-is (the zip-extraction path below uses names from inside the zip
      // itself, which Discord never touches).
      const restoredName = attachment.name.replace(/_/g, " ");
      ...
      await commitFileToRepo(`${normalizedFolder}/${restoredName}`, buffer);
      await interaction.editReply(
        `Added \`${restoredName}\` to \`${normalizedFolder}\`. It'll show up in the Discord ` +
